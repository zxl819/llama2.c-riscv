...
#ifndef PLAT_PRINT_USE_PRINTF
#define PLAT_PRINT_USE_PRINTF 0
#endif
...
static void print_uart_u64_hex(uint64_t val) {
    static const char *hex = "0123456789abcdef";
    print_uart("0x");
    for (int i = 15; i >= 0; i--) {
        uint8_t nib = (uint8_t)((val >> (i * 4)) & 0xFULL);
...
// Debug prints (0=off, 1=on). Keep output sparse to avoid slowing simulation.
#ifndef BARE_DEBUG
#define BARE_DEBUG 1
#endif

// Print generation progress every N tokens when BARE_DEBUG=1.
#ifndef BARE_DEBUG_EVERY
#define BARE_DEBUG_EVERY 8
#endif
...
// NOTE: crt.S places the stack immediately above `_end` (128KB per hart).
// If we also place the heap at `_end` and grow upwards, heap allocations will
// quickly collide with the stack and corrupt return addresses.
// To avoid this, we use a dedicated .bss heap buffer.
#ifndef BARE_HEAP_BYTES
#define BARE_HEAP_BYTES (192 * 1024 * 1024) // enlarged further for padded matmul + KV/cache allocations
#endif
...
static unsigned char bare_heap[BARE_HEAP_BYTES] __attribute__((aligned(BARE_ALLOC_ALIGN)));
static size_t bare_heap_offset = 0;

static void print_heap_usage(const char *tag) {
    if (!BARE_DEBUG) return;
    uintptr_t heap_base = (uintptr_t)bare_heap;
    uintptr_t heap_limit = (uintptr_t)bare_heap + (uintptr_t)BARE_HEAP_BYTES;
    uintptr_t used = (uintptr_t)bare_heap_offset;
    uintptr_t remaining = 0;
    if (heap_limit > heap_base + used) remaining = heap_limit - (heap_base + used);
    print_uart("[bare][heap] ");
    if (tag) print_uart(tag);
    print_uart(" used=");
    print_uart_int_dec((uint64_t)used);
    print_uart(" remaining=");
    print_uart_int_dec((uint64_t)remaining);
    print_uart("\r\n");
}
...
static void *bare_alloc(size_t size, size_t alignment) {
    ...
    uintptr_t cur = heap_base + (uintptr_t)bare_heap_offset;
    size_t misalignment = (size_t)(cur % (uintptr_t)alignment);
    if (misalignment != 0) cur += (uintptr_t)alignment - (uintptr_t)misalignment;

    if (cur + (uintptr_t)size > heap_limit) {
        print_uart("[bare] heap exhausted\r\n");
        print_uart("[bare] heap_base=");
        print_uart_u64_hex((uint64_t)heap_base);
        print_uart(" heap_limit=");
        print_uart_u64_hex((uint64_t)heap_limit);
        print_uart("\r\n");
        print_uart("[bare] cur=");
        print_uart_u64_hex((uint64_t)cur);
        print_uart(" size=");
        print_uart_int_dec((uint64_t)size);
        print_uart(" align=");
        print_uart_int_dec((uint64_t)alignment);
        print_uart("\r\n");
        print_uart("[bare] used=");
        print_uart_int_dec((uint64_t)bare_heap_offset);
        print_uart(" remaining=");
        if (heap_limit > heap_base + (uintptr_t)bare_heap_offset) {
            print_uart_int_dec((uint64_t)(heap_limit - (heap_base + (uintptr_t)bare_heap_offset)));
        } else {
            print_uart("0");
        }
        print_uart("\r\n");
        panic("bare heap exhausted");
        print_uart("\r\n");
        print_uart("[bare] heap_base=");
        print_uart_u64_hex((uint64_t)heap_base);
        print_uart(" heap_limit=");
    ...
    void *ptr = (void *)cur;
    bare_heap_offset = (size_t)((cur - heap_base) + (uintptr_t)size);
    return ptr;
}
...
// Override the weak handle_trap in crt.S so traps don't silently exit(-32).
uintptr_t handle_trap(uintptr_t mcause, uintptr_t mepc, uintptr_t sp) {
    ...
    print_uart("\r\n[TRAP] mcause=");
    print_uart_u64_hex((uint64_t)mcause);
    print_uart(" mepc=");
    print_uart_u64_hex((uint64_t)mepc);
    print_uart(" mtval=");
    print_uart_u64_hex((uint64_t)mtval);
    print_uart(" mstatus=");
    print_uart_u64_hex((uint64_t)mstatus);
    print_uart("\r\n");
...
static inline uint64_t rdcycle(void) {
    uint64_t c;
    // Use mcycle (machine counter) to avoid requiring Zicntr (cycle CSR).
...
float sqrtf(float x) {
    // IMPORTANT: do not use __builtin_sqrtf() in this freestanding build.
    // It may be lowered to a libcall to `sqrtf`, which then resolves back to
    // this symbol, turning into an infinite self-jump (observed as `j sqrtf`).
    //
    // Self-contained approximation via fast inverse square root (2 Newton steps).
    // Good enough for rmsnorm/attention scaling in this demo.
...
static int clamp_seq_len(int want, int max_seq_len) {
    if (want < 1) want = 1;
    if (want > max_seq_len) want = max_seq_len;
    return want;
}

void malloc_run_state(RunState* s, Config* p, int runtime_seq_len) {
    int kv_dim = (p->dim * p->n_kv_heads) / p->n_heads;
    s->seq_len = clamp_seq_len(runtime_seq_len, p->seq_len);
    if (BARE_DEBUG) {
        print_uart("[bare] malloc_run_state: dim=");
        print_uart_int_dec((uint64_t)p->dim);
        print_uart(" hidden=");
        print_uart_int_dec((uint64_t)p->hidden_dim);
        print_uart(" layers=");
        print_uart_int_dec((uint64_t)p->n_layers);
        print_uart(" heads=");
        print_uart_int_dec((uint64_t)p->n_heads);
        print_uart(" kv_heads=");
        print_uart_int_dec((uint64_t)p->n_kv_heads);
        print_uart(" vocab=");
        print_uart_int_dec((uint64_t)p->vocab_size);
        print_uart(" seq_len(model)=");
        print_uart_int_dec((uint64_t)p->seq_len);
        print_uart(" seq_len(run)=");
        print_uart_int_dec((uint64_t)s->seq_len);
        print_uart(" kv_dim=");
        print_uart_int_dec((uint64_t)kv_dim);
        print_uart("\r\n");

        uint64_t bytes_att = (uint64_t)p->n_heads * (uint64_t)s->seq_len * sizeof(float);
        uint64_t bytes_logits = (uint64_t)p->vocab_size * sizeof(float);
        uint64_t bytes_k = (uint64_t)p->n_layers * (uint64_t)s->seq_len * (uint64_t)kv_dim * sizeof(float);
        print_uart("[bare] alloc bytes: att=");
        print_uart_int_dec(bytes_att);
        print_uart(" logits=");
        print_uart_int_dec(bytes_logits);
        print_uart(" key_cache=");
        print_uart_int_dec(bytes_k);
        print_uart(" value_cache=");
        print_uart_int_dec(bytes_k);
        print_uart("\r\n");
    }
    ...
    s->att = calloc(p->n_heads * s->seq_len, sizeof(float));
    s->logits = calloc(p->vocab_size, sizeof(float));
    s->key_cache = calloc(p->n_layers * s->seq_len * kv_dim, sizeof(float));
    s->value_cache = calloc(p->n_layers * s->seq_len * kv_dim, sizeof(float));
    ...
    print_heap_usage("after RunState");
}
...
static void init_transformer_from_embedded(Transformer *t) {
    ...
    if (BARE_DEBUG) {
        print_uart("[bare] model cfg: dim=");
        print_uart_int_dec((uint64_t)t->config.dim);
        print_uart(" hidden=");
        print_uart_int_dec((uint64_t)t->config.hidden_dim);
        print_uart(" layers=");
        print_uart_int_dec((uint64_t)t->config.n_layers);
        print_uart(" heads=");
        print_uart_int_dec((uint64_t)t->config.n_heads);
        print_uart(" kv_heads=");
        print_uart_int_dec((uint64_t)t->config.n_kv_heads);
        print_uart(" vocab=");
        print_uart_int_dec((uint64_t)t->config.vocab_size);
        print_uart(" seq_len=");
        print_uart_int_dec((uint64_t)t->config.seq_len);
        print_uart(" shared=");
        print_uart_int_dec((uint64_t)shared);
        print_uart("\r\n");
    }

    float *weights_ptr = (float*)(MODEL_BIN_START + sizeof(Config));
    memory_map_weights(&t->weights, &t->config, weights_ptr, shared);
    // Allocate only as much KV cache/att as we will actually use.
    malloc_run_state(&t->state, &t->config, BARE_STEPS);
    if (BARE_DEBUG) print_uart("[bare] transformer init done\r\n");
}
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for (unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        for (int h = 0; h < p->n_heads; h++) {
            ...
            for (int t = pos + 1; t < seq_len; t++) att[t] = -1e9f;
            softmax(att, pos + 1);
...
static void build_tokenizer_from_embedded(Tokenizer* t, int vocab_size) {
    ...
    ptr += sizeof(int);
    if (BARE_DEBUG) {
        print_uart("[bare] tokenizer: vocab=");
        print_uart_int_dec((uint64_t)vocab_size);
        print_uart(" max_token_len=");
        print_uart_int_dec((uint64_t)t->max_token_length);
        print_uart("\r\n");
    }
    ...
    print_heap_usage("after tokenizer");
}
...
void build_sampler(Sampler* sampler, int vocab_size, float temperature, float topp, unsigned long long seed) {
    ...
    print_heap_usage("after sampler");
}
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
    ...
    if (BARE_DEBUG) {
        print_uart("[bare] generate: steps=");
        print_uart_int_dec((uint64_t)steps);
        print_uart(" prompt_len=");
        print_uart_int_dec((uint64_t)strlen(prompt));
        print_uart("\r\n");
    }
    ...
    if (BARE_DEBUG) {
        print_uart("[bare] prompt_tokens=");
        print_uart_int_dec((uint64_t)num_prompt_tokens);
        print_uart(" first=");
        print_uart_int_dec((uint64_t)prompt_tokens[0]);
        print_uart("\r\n");
    }
    ...
    while (pos < steps) {
        ...
        if (BARE_DEBUG && (pos % BARE_DEBUG_EVERY) == 0) {
            print_uart("[bare] step pos=");
            print_uart_int_dec((uint64_t)pos);
            print_uart(" token=");
            print_uart_int_dec((uint64_t)token);
            print_uart(" next=");
            print_uart_int_dec((uint64_t)next);
            print_uart("\r\n");
        }
...
int main(void) {
    ...
    print_uart("[bare] heap_start=");
    print_uart_u64_hex((uint64_t)(uintptr_t)bare_heap);
    print_uart(" heap_end=");
    print_uart_u64_hex((uint64_t)(uintptr_t)bare_heap + (uint64_t)BARE_HEAP_BYTES);
    print_uart("\r\n");
    print_uart("[bare] heap_bytes=");
    print_uart_int_dec((uint64_t)BARE_HEAP_BYTES);
    print_uart("\r\n");
