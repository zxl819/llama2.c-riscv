...
static void __attribute__((noinline)) vle32debug(float* xout, float* w1, float* w2, int n, int d) {
    ...
    for (int i = 0; i < d; i++) {
        ...
        while (avl > 0) {
            ...
            // === 调试点 3: 检查即将加载的内存内容 ===
            print_uart("DEBUG: About to load from row1. First 16 elements: ");
            for (int k = 0; k < 16; ++k) {
            ...
            print_uart("DEBUG: About to load from row2. First 16 elements: ");
            for (int k = 0; k < 16; ++k) {
...
int real_main() {
   ...
   int8_t xout[d];
    ...
    for (int i = 0; i < d*n; i++) {
        print_uart_float(w1[i]);
        print_uart(" ");
        if ((i + 1) % 16 == 0) print_uart("\r\n");
        //print_uart("\r\n");
    }

    print_uart("Input Matrix w2:\r\n");
    for (int i = 0; i < d; i++) {
        print_uart_float(w2[i]);
        print_uart(" ");
        if ((i + 1) % 16 == 0) print_uart("\r\n");
        //print_uart("\r\n");
    }
    ...
    vle32debug(xout, &w1[0], &w2[0], n, d);
    print_uart("vledebug end:\r\n");
