...
static void __attribute__((noinline)) vle32debug(float* xout, float* w1, float* w2, int n, int d) {
    ...
    for (int i = 0; i < d; i++) {
        ...
        while (avl > 0) {
            ...
            // Load vectors
            vfloat32m1_t vrow1 = __riscv_vle32_v_f32m1(row1, vl);
            vfloat32m1_t vrow2 = __riscv_vle32_v_f32m1(row2, vl); 
            print_uart("DEBUG: About to vle32.v from row2 ptr: 0x");
            print_uart_hex((unsigned long long)row2);
            print_uart("\r\n");
            debug_delay_cycles(50);
            // Debug: Verify vrow load
            static float debug_vec1[16]; // 16 is enough for VL=512 (512/32=16)
            static float debug_vec2[16]; // 16 is enough for VL=512 (512/32=16)
            __riscv_vse32_v_f32m1(debug_vec1, vrow1, vl);
            __riscv_vse32_v_f32m1(debug_vec2, vrow2, vl);
            //__riscv_vse32_v_f32m1(&temp[temp_idx], vrow, vl);
            //asm volatile("fence rw,rw" ::: "memory");
            debug_delay_cycles(50);
            print_uart("    Checking w1 (vec vs mem): \r\n");
            for (size_t k = 0; k < vl; k++) {
                float vec_val = debug_vec1[k];
            ...
            //vfloat32m1_t vrow2 = __riscv_vle32_v_f32m1(row2, vl); 
            print_uart("    Checking w2 (vec vs mem): \r\n");
             for (size_t k = 0; k < vl; k++) {
                float vec_val = debug_vec2[k];
