...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for (unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        TIME_OP(pos, "w1", PRINT_DIMS_2(hidden_dim, dim), MATMUL_Q_TAGGED("ffn", (int)l, s->hb, &s->xq, w->w1_q + l, dim, hidden_dim));
        TIME_OP(pos, "w3", PRINT_DIMS_2(hidden_dim, dim), MATMUL_Q_TAGGED("ffn", (int)l, s->hb2, &s->xq, w->w3_q + l, dim, hidden_dim));
        TIME_OP(pos, "swiglu", PRINT_DIMS_1(hidden_dim),
        ...
        TIME_OP(pos, "w2", PRINT_DIMS_2(dim, hidden_dim), MATMUL_Q_TAGGED("ffn", (int)l, s->xb, &s->hq, w->w2_q + l, hidden_dim, dim));
        ...
        TIME_OP(pos, "wcls", PRINT_DIMS_2(p->vocab_size, p->dim), MATMUL_Q_TAGGED("wcls", -1, s->logits, &s->xq, &w->wcls_q, p->dim, p->vocab_size));
        return s->logits;
    }
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
...
#if BARE_USE_CYCLE_COUNTER
    print_uart("[bare] strlen(prompt): ");
    print_uart_int_dec((uint64_t)strlen(prompt));
    print_uart("\r\n");
#endif
    TIME_OP(-1, "encode", PRINT_NO_DIMS, encode(tokenizer, prompt, 1, 0, prompt_tokens, &num_prompt_tokens));
    if (num_prompt_tokens < 1) panic("prompt encode empty");
#if BARE_USE_CYCLE_COUNTER
    print_uart("[bare] num_prompt_tokens: ");
    ...
    while (pos < steps) {
        ...
        if (pos < num_prompt_tokens - 1) {
            ...
            TIME_OP(pos, "sample", PRINT_DIMS_1(transformer->config.vocab_size), next = sample_token(sampler, logits));
            #if BARE_USE_CYCLE_COUNTER
        ...
        if (next == 1) break;
        char* piece;
        TIME_OP(pos, "decode", PRINT_NO_DIMS, piece = decode(tokenizer, token, next));
        //safe_printf(piece);
        TIME_OP(pos, "print", PRINT_NO_DIMS,
        if (piece) {
            int should_print = 1;
