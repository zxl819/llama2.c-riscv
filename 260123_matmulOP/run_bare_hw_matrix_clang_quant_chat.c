...
void transformer_prefill(Transformer *t, int* tokens, int n_tokens) {
    ...
    // NOTE: Batch buffers are not freed ... leak memory warning ...
    free(x_batch); free(xb_batch); free(xb2_batch);
    free(q_batch); free(k_batch); free(v_batch);
    free(hb_batch); free(hb2_batch);
    free(xq_batch_q); free(xq_batch_s);
    free(hq_batch_q); free(hq_batch_s);
}
...
void chat(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, int steps) {
    ...
    if (!prompt_tokens) panic("chat alloc");
    
    int token, next;
    int pos = 0;

    print_uart("Llama2 Chat (Bare Metal)\r\n");
    print_uart("Type your message and press Enter.\r\n");
