...
// ---
// [代码说明]
// 本文件是 Llama2 模型在嵌入式裸机环境下的推理引擎核心入口。
// 主要功能：
// 1. 实现模型权重与分词器的二进制嵌入加载逻辑（通过链接脚本符号 MODEL_BIN_START 等）。
// 2. 调度 Transformer 的 Forward 流程，支持基于 AME (Matrix) 和 RVV (Vector) 的全量化推理。
// 3. 集成了 UART 输出与基础的 CPU 周期计数统计（RD_CYCLE）。
// 4. 实现了一个微型的 BUMP 分配器 (bare_heap) 用于运行时内存管理。
