...
#define BARE_MEM_STATS 0

// Heap allocation trace: prints bare_heap_offset and requested size on every allocation.
// Set to 0 to disable (very verbose).
#ifndef BARE_HEAP_TRACE
#define BARE_HEAP_TRACE 0
#endif
...
// ---------------------------------------------------------------------------
// 64B-aligned allocation helpers for large / matrix-friendly buffers.
// Keep default malloc/calloc at 8B to avoid wasting heap on many small objects
// (e.g., tokenizer strings).
#ifndef BARE_BIG_ALLOC_ALIGNMENT
#define BARE_BIG_ALLOC_ALIGNMENT 64
#endif

static void *bare_alloc(size_t size, size_t alignment);
void *memset(void *dest, int byte, size_t len);

static void *bare_malloc64(size_t size) {
    if (size == 0) size = 1;
    return bare_alloc(size, BARE_BIG_ALLOC_ALIGNMENT);
}

static void *bare_calloc64(size_t count, size_t size) {
    if (count == 0 || size == 0) return bare_malloc64(1);
    size_t total = count * size;
    void *ptr = bare_malloc64(total);
    memset(ptr, 0, total);
    return ptr;
}
...
static QuantizedTensor qt_alloc(int n) {
    QuantizedTensor t;
    t.q = (int8_t*)bare_calloc64((size_t)n, sizeof(int8_t));
    t.s = (float*)bare_calloc64((size_t)((n + GS - 1) / GS), sizeof(float));
    if (!t.q || !t.s) {
...
#define BARE_HEAP_BYTES 0x01580000  
#endif
//0x01580000 21.5MB
//0x015D0000  21.8MB
#ifndef BARE_CPU_HZ
...
#ifndef BARE_MALLOC_ALIGNMENT
#define BARE_MALLOC_ALIGNMENT 8
#endif

// Use the linker-defined SRAM heap window instead of a fixed-size static array.
// This avoids wasting SRAM in `.bss` and lets us use the maximum available
// space between the end of `.bss` and the stack bottom.
extern unsigned char __heap_start[];
extern unsigned char __heap_end[];

static unsigned char *bare_heap = NULL;
static size_t bare_heap_bytes = 0;
static size_t bare_heap_offset = 0;

static void bare_heap_init(void) {
    if (bare_heap) return;
    bare_heap = __heap_start;
    bare_heap_bytes = (size_t)(__heap_end - __heap_start);
}
...
static void panic(const char *msg) {
    ...
    // Heap window diagnostics (from linker script)
    bare_heap_init();
    print_uart("Heap window: start=0x");
    print_uart_hex((unsigned long long)(uintptr_t)__heap_start);
    print_uart(" end=0x");
    print_uart_hex((unsigned long long)(uintptr_t)__heap_end);
    print_uart(" bytes=");
    print_uart_int_dec((unsigned long long)bare_heap_bytes);
    print_uart("\r\n");
    exit(1);
...
#if BARE_HEAP_TRACE
static int g_uart_inited_for_trace = 0;
static inline void heap_trace_ensure_uart(void) {
    if (!g_uart_inited_for_trace) {
        init_uart(CLOCK_FREQUENCY, UART_BITRATE);
        g_uart_inited_for_trace = 1;
    }
}
#endif
...
static void *bare_alloc(size_t size, size_t alignment) {
    bare_heap_init();
#if BARE_HEAP_TRACE
    heap_trace_ensure_uart();
    print_uart("[bare][alloc] off=");
    print_uart_int_dec((unsigned long long)bare_heap_offset);
    print_uart(" size=");
    print_uart_int_dec((unsigned long long)size);
    print_uart("\r\n");
#endif
    ...
    if (bare_heap_offset + size > bare_heap_bytes) {
        //init_uart(CLOCK_FREQUENCY, UART_BITRATE);
        print_uart("[bare] Heap Exhausted! Used: ");
        print_uart_int_dec((unsigned long long)bare_heap_offset);
        print_uart(" Requested: ");
        print_uart_int_dec((unsigned long long)size);
        print_uart(" Limit: ");
        print_uart_int_dec((unsigned long long)bare_heap_bytes);
        print_uart("\r\n");
...
void *malloc(size_t size) {
    if (size == 0) size = 1;
    return bare_alloc(size, BARE_MALLOC_ALIGNMENT);
}

void *calloc(size_t count, size_t size) {
    if (count == 0 || size == 0) return bare_alloc(1, BARE_MALLOC_ALIGNMENT);
    size_t total = count * size;
    void *ptr = bare_alloc(total, BARE_MALLOC_ALIGNMENT);
    memset(ptr, 0, total);
    return ptr;
}
...
void malloc_run_state(RunState* s, Config* p) {
    ...
    s->x = bare_calloc64((size_t)p->dim, sizeof(float));
    s->xb = bare_calloc64((size_t)p->dim, sizeof(float));
    s->xb2 = bare_calloc64((size_t)p->dim, sizeof(float));
    s->hb = bare_calloc64((size_t)p->hidden_dim, sizeof(float));
    s->hb2 = bare_calloc64((size_t)p->hidden_dim, sizeof(float));
    s->xq = qt_alloc(p->dim);
    s->hq = qt_alloc(p->hidden_dim);
    s->q = bare_calloc64((size_t)p->dim, sizeof(float));
    s->k = bare_calloc64((size_t)p->dim, sizeof(float));
    s->v = bare_calloc64((size_t)p->dim, sizeof(float));
    s->att = bare_calloc64((size_t)p->n_heads * (size_t)p->seq_len, sizeof(float));
    s->logits = bare_calloc64((size_t)p->vocab_size, sizeof(float));
    s->key_cache = bare_calloc64((size_t)p->n_layers * (size_t)p->seq_len * (size_t)kv_dim, sizeof(float));
    s->value_cache = bare_calloc64((size_t)p->n_layers * (size_t)p->seq_len * (size_t)kv_dim, sizeof(float));
...
void transformer_prefill(Transformer *t, int* tokens, int n_tokens) {
    ...
    float* x_batch = (float*)bare_malloc64((size_t)n_tokens * (size_t)dim * sizeof(float));
    float* xb_batch = (float*)bare_malloc64((size_t)n_tokens * (size_t)dim * sizeof(float));
    float* xb2_batch = (float*)bare_malloc64((size_t)n_tokens * (size_t)dim * sizeof(float));
    float* q_batch = (float*)bare_malloc64((size_t)n_tokens * (size_t)dim * sizeof(float));
    float* k_batch = (float*)bare_malloc64((size_t)n_tokens * (size_t)kv_dim * sizeof(float));
    float* v_batch = (float*)bare_malloc64((size_t)n_tokens * (size_t)kv_dim * sizeof(float));
    float* hb_batch = (float*)bare_malloc64((size_t)n_tokens * (size_t)hidden_dim * sizeof(float));
    float* hb2_batch = (float*)bare_malloc64((size_t)n_tokens * (size_t)hidden_dim * sizeof(float));
    
    int8_t* xq_batch_q = (int8_t*)bare_malloc64((size_t)n_tokens * (size_t)dim * sizeof(int8_t));
    float* xq_batch_s = (float*)bare_malloc64((size_t)n_tokens * (size_t)num_groups_x * sizeof(float));
    int8_t* hq_batch_q = (int8_t*)bare_malloc64((size_t)n_tokens * (size_t)hidden_dim * sizeof(int8_t));
    float* hq_batch_s = (float*)bare_malloc64((size_t)n_tokens * (size_t)num_groups_h * sizeof(float));
    ...
    for (int l = 0; l < p->n_layers; l++) {
        //print_uart("\r\n[bare] layer="); print_uart_int_dec((uint64_t)l); print_uart("\r\n");
        // RMSNorm (Att)
        for (int i = 0; i < n_tokens; i++) {
...
#if MATRIX_KERNEL_DEBUG_PRINT
        print_uart("[bare] matmul: q layer="); print_uart_int_dec((uint64_t)l); print_uart("\r\n");
#endif
        matmul_q_batch(q_batch, xq_batch_q, xq_batch_s, w->wq_q[l].q, w->wq_q[l].s, dim, dim, n_tokens);
#if MATRIX_KERNEL_DEBUG_PRINT
        print_uart("[bare] matmul: k layer="); print_uart_int_dec((uint64_t)l); print_uart("\r\n");
#endif
        matmul_q_batch(k_batch, xq_batch_q, xq_batch_s, w->wk_q[l].q, w->wk_q[l].s, dim, kv_dim, n_tokens);
#if MATRIX_KERNEL_DEBUG_PRINT
        print_uart("[bare] matmul: v layer="); print_uart_int_dec((uint64_t)l); print_uart("\r\n");
#endif
        ...
        quantize_batch(xq_batch_q, xq_batch_s, xb_batch, dim, n_tokens);
#if MATRIX_KERNEL_DEBUG_PRINT
        print_uart("[bare] matmul: wo layer="); print_uart_int_dec((uint64_t)l); print_uart("\r\n");
#endif
        ...
        quantize_batch(xq_batch_q, xq_batch_s, xb_batch, dim, n_tokens);
#if MATRIX_KERNEL_DEBUG_PRINT
        print_uart("[bare] matmul: w1 layer="); print_uart_int_dec((uint64_t)l); print_uart("\r\n");
#endif
        matmul_q_batch(hb_batch, xq_batch_q, xq_batch_s, w->w1_q[l].q, w->w1_q[l].s, dim, hidden_dim, n_tokens);
#if MATRIX_KERNEL_DEBUG_PRINT
        print_uart("[bare] matmul: w3 layer="); print_uart_int_dec((uint64_t)l); print_uart("\r\n");
#endif
...
#if MATRIX_KERNEL_DEBUG_PRINT
        print_uart("[bare] matmul: w2 layer="); print_uart_int_dec((uint64_t)l); print_uart("\r\n");
#endif
