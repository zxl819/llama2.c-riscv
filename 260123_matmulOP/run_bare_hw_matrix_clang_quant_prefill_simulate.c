...
static void matmul_q(float* xout, const QuantizedTensor *x, const QuantizedTensor *w, int n, int d) {
    // TRACE_STACK(); // Too noisy for per-matmul, uncomment if needed
    // for (int i = 0; i < d; i++) {
    //     float val = 0.0f;
    //     int32_t ival = 0;
    //     int in = i * n;
...
float* forward(Transformer* transformer, int token, int pos) {
    if (pos == 0) TRACE_STACK(); // Only print on first token to avoid spam
    #if MATRIX_KERNEL_DEBUG_PRINT
...
static void matmul_q_batch(float* xout, const int8_t* xq, const float* xs, 
    ...
    // TRACE_STACK(); // Optional, usually very deep
...
extern char _end[]; // Linker symbol for end of BSS provided by linker script/default
static const unsigned long STACK_SIZE_TOTAL = 1024 * 1024; // 1MB from crt_uart.S

static void trace_stack_usage(const char* func_name) {
    uintptr_t stack_top_initial = (uintptr_t)_end + STACK_SIZE_TOTAL;
    uintptr_t sp = get_sp();
    if (sp < stack_top_initial) {
        print_uart("[stack] ");
        print_uart(func_name);
        print_uart(": used ");
        print_uart_int_dec((unsigned long long)(stack_top_initial - sp));
        print_uart(" bytes\r\n");
    }
}

#define TRACE_STACK() trace_stack_usage(__func__)

static void print_simulated_memory_usage(Transformer *t) {
    Config *p = &t->config;
    unsigned long long total_quant_bytes = 0;
    unsigned long long run_state_bytes = 0;
    ...
    // Helper macro to calculate bytes for a QuantizedTensor of size n
    #define CALC_QT_BYTES(n) ((unsigned long long)(n) * sizeof(int8_t) + ((unsigned long long)((n) + GS - 1) / GS) * sizeof(float))

    // Weights (per layer)
    for(int i=0; i<n_layers; i++) {
        // wq: dim * dim
        total_quant_bytes += CALC_QT_BYTES(dim * dim);
        // wk: dim * kv_dim
        total_quant_bytes += CALC_QT_BYTES(dim * kv_dim);
        // wv: dim * kv_dim
        total_quant_bytes += CALC_QT_BYTES(dim * kv_dim);
        // wo: dim * dim
        total_quant_bytes += CALC_QT_BYTES(dim * dim);
        // w1: dim * hidden_dim
        total_quant_bytes += CALC_QT_BYTES(dim * hidden_dim);
        // w2: hidden_dim * dim
        total_quant_bytes += CALC_QT_BYTES(hidden_dim * dim);
        // w3: dim * hidden_dim
        total_quant_bytes += CALC_QT_BYTES(dim * hidden_dim);
    }
    
    // wcls: dim * vocab_size
    total_quant_bytes += CALC_QT_BYTES(dim * vocab_size);

    // RunState Buffers
    // float buffers in malloc_run_state:
    // x, xb, xb2, q, k, v are size dim
    run_state_bytes += (unsigned long long)dim * sizeof(float) * 6;
    // hb, hb2 are size hidden_dim
    run_state_bytes += (unsigned long long)hidden_dim * sizeof(float) * 2;
    // att: n_heads * seq_len
    run_state_bytes += (unsigned long long)p->n_heads * seq_len * sizeof(float);
    // logits: vocab_size
    run_state_bytes += (unsigned long long)vocab_size * sizeof(float);
    // key_cache, value_cache: n_layers * seq_len * kv_dim
    run_state_bytes += (unsigned long long)n_layers * seq_len * kv_dim * sizeof(float) * 2;

    // Quantized buffers/structs in RunState:
    // xq -> dim
    run_state_bytes += CALC_QT_BYTES(dim);
    // hq -> hidden_dim
    run_state_bytes += CALC_QT_BYTES(hidden_dim);

    #undef CALC_QT_BYTES

    print_uart("[bare] Total QuantizedTensor size: ");
    print_uart_int_dec(total_quant_bytes);
    print_uart(" bytes\r\n");

    print_uart("[bare] Total RunState size: ");
    print_uart_int_dec(run_state_bytes);
    print_uart(" bytes\r\n");

    // Prefill temporary buffer calculation (per token)
    // See transformer_prefill() allocations
    unsigned long long prefill_per_token = 0;
    // float buffers: x, xb, xb2, q (dim) + k, v (kv_dim) + hb, hb2 (hidden_dim)
    prefill_per_token += (unsigned long long)(dim * 4 + kv_dim * 2 + hidden_dim * 2) * sizeof(float);
    // quant buffers: xq_q (dim), xq_s (groups_x), hq_q (hidden_dim), hq_s (groups_h)
    // We approximate roughly. groups ~ dim/GS
    prefill_per_token += (unsigned long long)dim * sizeof(int8_t); // xq_batch_q
    prefill_per_token += (unsigned long long)((dim + GS - 1) / GS) * sizeof(float); // xq_batch_s
    prefill_per_token += (unsigned long long)hidden_dim * sizeof(int8_t); // hq_batch_q
    prefill_per_token += (unsigned long long)((hidden_dim + GS - 1) / GS) * sizeof(float); // hq_batch_s

    print_uart("[bare] Prefill temp memory: ");
    print_uart_int_dec(prefill_per_token);
    print_uart(" bytes per prompt token (allocated on heap)\r\n");
...
// #endif
    ...
    print_simulated_memory_usage(&transformer);
