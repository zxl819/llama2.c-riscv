...
uintptr_t handle_trap(uintptr_t cause, uintptr_t epc, uintptr_t regs[32]) {
    ...
    for (int off = -1; off <= 4; ++off) {
        ...
        // Safe check for alignment before loading to avoid nested traps
        if (addr & 3) {
            print_uart("UNALIGNED!");
        } else {
...
#define BARE_HEAP_BYTES (200 * 1024 * 1024)
#endif
...
static void *bare_alloc(size_t size, size_t alignment) {
    ...
    if (bare_heap_offset + size > bare_heap_bytes) {
        print_uart("[bare] PANIC: bare heap exhausted\r\n");
        print_uart("Requested size: ");
        print_uart_int_dec((unsigned long long)size);
        print_uart(" bytes\r\n");
        print_heap_usage();
        exit(1);
    }
    void *ptr = &bare_heap_ptr[bare_heap_offset];
...
static void init_transformer_from_embedded(Transformer *t) {
    ...
    print_uart("[bare] Config: dim="); print_uart_int_dec((uint64_t)t->config.dim);
    print_uart(" hidden_dim="); print_uart_int_dec((uint64_t)t->config.hidden_dim);
    print_uart(" n_layers="); print_uart_int_dec((uint64_t)t->config.n_layers);
    print_uart(" n_heads="); print_uart_int_dec((uint64_t)t->config.n_heads);
    print_uart(" n_kv_heads="); print_uart_int_dec((uint64_t)t->config.n_kv_heads);
    print_uart(" vocab_size="); print_uart_int_dec((uint64_t)t->config.vocab_size);
    print_uart(" seq_len="); print_uart_int_dec((uint64_t)t->config.seq_len);
    print_uart("\r\n");
    ...
    // This assumes the embedded checkpoint stores float weights (v1 format).
    {
        ...
        //print_uart("[bare] Starting quantization loop...\r\n");
        for (int l = 0; l < p->n_layers; l++) {
            print_uart("  L"); print_uart_int_dec((uint64_t)l); print_uart(" ");
            // wq: (dim, dim)
            {
                const int n = dim;
                const int d = dim;
