...
static void matmul_q(float* xout, const QuantizedTensor *x, const QuantizedTensor *w, int n, int d) {
    // W (d,n) @ x (n,) -> xout (d,)
    // both x and w are per-group int8 quantized with group size GS
    for (int i = 0; i < d; i++) {
        float val = 0.0f;
        int32_t ival = 0;
        int in = i * n;
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for (unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        matmul_q(s->q, &s->xq, w->wq_q + l, dim, dim);
        matmul_q(s->k, &s->xq, w->wk_q + l, dim, kv_dim);
        matmul_q(s->v, &s->xq, w->wv_q + l, dim, kv_dim);
        ...
        for (int h = 0; h < p->n_heads; h++) {
            ...
            quantize(&s->xq, s->xb, dim);
            matmul_q(s->xb2, &s->xq, w->wo_q + l, dim, dim);
            for (int i = 0; i < dim; i++) x[i] = x[i] + s->xb2[i];

            rmsnorm(s->xb, x, w->rms_ffn_weight + l*dim, dim);
            quantize(&s->xq, s->xb, dim);
            matmul_q(s->hb, &s->xq, w->w1_q + l, dim, hidden_dim);
            matmul_q(s->hb2, &s->xq, w->w3_q + l, dim, hidden_dim);
            ...
            matmul_q(s->xb, &s->hq, w->w2_q + l, hidden_dim, dim);
            for (int i = 0; i < dim; i++) x[i] = x[i] + s->xb[i];
        }

        rmsnorm(x, x, w->rms_final_weight, dim);
        quantize(&s->xq, x, dim);
        matmul_q(s->logits, &s->xq, &w->wcls_q, p->dim, p->vocab_size);
        return s->logits;
    }
