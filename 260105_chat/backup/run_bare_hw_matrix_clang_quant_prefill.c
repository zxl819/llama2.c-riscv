...
static void BARE_NO_AUTOVEC matmul_q(float* xout, const QuantizedTensor *x, const QuantizedTensor *w, int n, int d) {
    // One-time timing: measure latency from boot to first matmul_q entry.
// #if BARE_USE_CYCLE_COUNTER
//     static int first_matmul_q_printed = 0;
//     extern uint64_t g_bare_boot_cycles;
//     if (!first_matmul_q_printed) {
//         first_matmul_q_printed = 1;
//         uint64_t now = rdcycle();
//         uint64_t delta = now - g_bare_boot_cycles;
//         print_uart("[bare] cycles to first matmul_q: ");
//         print_uart_int_dec(delta);
//         long ms = cycles_to_ms(g_bare_boot_cycles, now);
//         if (ms > 0) {
//             print_uart(" (");
//             print_uart_int_dec((uint64_t)ms);
//             print_uart(" ms)\r\n");
//         } else {
//             print_uart("\r\n");
//         }
...
void malloc_run_state(RunState* s, Config* p) {
    ...
    // Allocate for max sequence length to support full-batch prefill
    int max_batch = p->seq_len;
    
    s->x = calloc(p->dim * max_batch, sizeof(float));
    s->xb = calloc(p->dim * max_batch, sizeof(float));
    s->xb2 = calloc(p->dim * max_batch, sizeof(float));
    s->hb = calloc(p->hidden_dim * max_batch, sizeof(float));
    s->hb2 = calloc(p->hidden_dim * max_batch, sizeof(float));
    s->xq = qt_alloc(p->dim * max_batch);
    s->hq = qt_alloc(p->hidden_dim * max_batch);
    s->q = calloc(p->dim * max_batch, sizeof(float));
    s->k = calloc(p->dim * max_batch, sizeof(float));
    s->v = calloc(p->dim * max_batch, sizeof(float));
    s->att = calloc(max_batch * p->n_heads * p->seq_len, sizeof(float));
...
static void BARE_NO_AUTOVEC matmul_q_batch(float* xout, const QuantizedTensor *x, const QuantizedTensor *w, int n, int d, int batch) {
     matrix_kernel_qmatmul_f32_noblk_batch(xout, x->q, x->s, w->q, w->s, n, d, GS, n, batch);
}
...
void quantize_batch(QuantizedTensor *qx, float* x, int n, int batch) {
    int n_groups = (n + GS - 1) / GS;
    for(int b=0; b<batch; ++b) {
        QuantizedTensor q_item;
        q_item.q = qx->q + b * n;
        q_item.s = qx->s + b * n_groups;
        quantize_const(&q_item, x + b * n, n);
    }
}

float* forward_batch(Transformer* transformer, int* tokens, int n_tokens, int pos_start) {
    #if MATRIX_KERNEL_DEBUG_PRINT
    print_uart("[bare] enter forward_batch...\r\n");
    #endif
    ...
    // Load embeddings for all tokens
    for(int i=0; i<n_tokens; ++i) {
        float* content_row = w->token_embedding_table + tokens[i] * dim;
        memcpy(s->x + i*dim, content_row, dim * sizeof(float));
    }

    for (unsigned long long l = 0; l < p->n_layers; l++) {
        // RMSNorm
        for(int i=0; i<n_tokens; ++i) {
            rmsnorm(s->xb + i*dim, s->x + i*dim, w->rms_att_weight + l*dim, dim);
        }

        // Quantize batch
        quantize_batch(&s->xq, s->xb, dim, n_tokens);

        // QKV Matmuls (Batched)
        matmul_q_batch(s->q, &s->xq, w->wq_q + l, dim, dim, n_tokens);
        matmul_q_batch(s->k, &s->xq, w->wk_q + l, dim, kv_dim, n_tokens);
        matmul_q_batch(s->v, &s->xq, w->wv_q + l, dim, kv_dim, n_tokens);

        // RoPE (per token)
        for(int start=0; start<n_tokens; ++start) {
            int pos = pos_start + start;
            int offset = start * dim;
            int offset_kv = start * kv_dim;
            ...
            for (int i = 0; i < dim; i += 2) {
                ...
                float q0 = s->q[offset + i];
                float q1 = s->q[offset + i+1];
                s->q[offset + i]   = q0 * fcr - q1 * fci;
                s->q[offset + i+1] = q0 * fci + q1 * fcr;
                
                if (i < kv_dim) {
                    float k0 = s->k[offset_kv + i];
                    float k1 = s->k[offset_kv + i+1];
                    s->k[offset_kv + i]   = k0 * fcr - k1 * fci;
                    s->k[offset_kv + i+1] = k0 * fci + k1 * fcr;
                }
            }
        }
        ...
        for(int start=0; start<n_tokens; ++start) {
            ...
            memcpy(k_slot, s->k + start*kv_dim, kv_dim * sizeof(float));
            memcpy(v_slot, s->v + start*kv_dim, kv_dim * sizeof(float));
        }

        // Multi-head Attention
        for(int start=0; start<n_tokens; ++start) {
            int pos = pos_start + start;
            int offset_q = start * dim;
            int offset_xb = start * dim;
            float* att_base = s->att + start * (p->n_heads * p->seq_len);

            for (int h = 0; h < p->n_heads; h++) {
                float* q = s->q + offset_q + h * head_size;
                float* att = att_base + h * p->seq_len;
                ...
                float* xb_head = s->xb + offset_xb + h * head_size;
                for (int i = 0; i < head_size; i++) xb_head[i] = 0.0f;
                for (int t = 0; t <= pos; t++) {
                    float att_t = att[t];
                    float* v = layer_val_cache + t * kv_dim + (h/kv_mul) * head_size;
                    for (int i = 0; i < head_size; i++) xb_head[i] += att_t * v[i];
                }
            }
        }
        ...
        // Output proj
        quantize_batch(&s->xq, s->xb, dim, n_tokens);
        matmul_q_batch(s->xb2, &s->xq, w->wo_q + l, dim, dim, n_tokens);
        
        // Residual
        for(int i=0; i<n_tokens * dim; ++i) s->x[i] += s->xb2[i];

        // FFN
        for(int i=0; i<n_tokens; ++i) {
            rmsnorm(s->xb + i*dim, s->x + i*dim, w->rms_ffn_weight + l*dim, dim);
        }
        quantize_batch(&s->xq, s->xb, dim, n_tokens);
        
        matmul_q_batch(s->hb, &s->xq, w->w1_q + l, dim, hidden_dim, n_tokens);
        matmul_q_batch(s->hb2, &s->xq, w->w3_q + l, dim, hidden_dim, n_tokens);
        ...
        quantize_batch(&s->hq, s->hb, hidden_dim, n_tokens);
        matmul_q_batch(s->xb, &s->hq, w->w2_q + l, hidden_dim, dim, n_tokens);
        
        // Residual
        for(int i=0; i<n_tokens * dim; ++i) s->x[i] += s->xb[i];
    }
    
    // We only compute logits for the LAST token in the batch
    int last_idx = n_tokens - 1;
    rmsnorm(s->x + last_idx*dim, s->x + last_idx*dim, w->rms_final_weight, dim);
    quantize(&s->xq, s->x + last_idx*dim, dim);
    matmul_q(s->logits, &s->xq, &w->wcls_q, dim, p->vocab_size);
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
    ...
    if (num_prompt_tokens > 0) {
#if MATRIX_KERNEL_DEBUG_PRINT
        print_uart("[bare] prefill batch...\r\n");
#endif
        // Print prompt tokens immediately for UX
        for (int i = 0; i < num_prompt_tokens - 1; i++) {
             char* piece = decode(tokenizer, prompt_tokens[i], prompt_tokens[i+1]);
             if (piece) {
                // Inline simple print loop or call safe_printf/logic
                // Reusing the heavy logic from loop is hard unless I extract it.
                // For now, use safe_printf because extracting `word_buffer` logic is verbose.
                // Note: word wrapping state (line_char_count) won't be perfectly sync'd if I mix safe_printf and complex logic.
                // But prompt is usually short.
                safe_printf(piece);
             }
        }
        ...
        float* logits = forward_batch(transformer, prompt_tokens, num_prompt_tokens, 0);
        
        // Sample first new token
        int next = sample_token(sampler, logits);
        ...
        char* piece = decode(tokenizer, prompt_tokens[num_prompt_tokens-1], next);
        safe_printf(piece);
    ...
    while (pos < steps) {
        ...
        int next;
        // Prompt handling removed from loop since we did it in batch
        // But if steps > num_prompt_tokens, we continue generating.
        // We are already at pos = num_prompt_tokens.
        // So next is sampled always.
