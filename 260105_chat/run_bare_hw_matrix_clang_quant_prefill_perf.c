...
static void matmul_q(float* xout, const QuantizedTensor *x, const QuantizedTensor *w, int n, int d) {
    ...
    // }
     ...
     matrix_kernel_qmatmul_f32_noblk_CT(xout, x->q, x->s, w->q, w->s, n, d, GS, n);
}
// ---------------------------------------------------------------------------
...
#define PRINT_DIMS_1(x) { print_uart("("); print_uart_int_dec((uint64_t)(x)); print_uart(")"); }
#define PRINT_DIMS_2(x, y) { print_uart("("); print_uart_int_dec((uint64_t)(x)); print_uart(","); print_uart_int_dec((uint64_t)(y)); print_uart(")"); }
#define PRINT_DIMS_3(x, y, z) { print_uart("("); print_uart_int_dec((uint64_t)(x)); print_uart(","); print_uart_int_dec((uint64_t)(y)); print_uart(","); print_uart_int_dec((uint64_t)(z)); print_uart(")"); }
#define PRINT_NO_DIMS { }

#if BARE_USE_CYCLE_COUNTER
#define TIME_OP(pos, name, print_dims, code) \
    do { \
        uint64_t start = rdcycle(); \
        code; \
        uint64_t end = rdcycle(); \
        uint64_t delta = end - start; \
        print_uart("[prof] token "); \
        ...
        print_dims; \
        print_uart(": "); \
        print_uart_int_dec(delta); \
        print_uart(" cycles ("); \
        print_uart_int_dec((uint64_t)cycles_to_ms(start, end)); \
        print_uart(" ms)\r\n"); \
    } while (0)
#else
#define TIME_OP(pos, name, print_dims, code) code
#endif
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    TIME_OP(pos, "memcpy_embed", PRINT_DIMS_1(dim), memcpy(x, content_row, dim * sizeof(*x)));

    for (unsigned long long l = 0; l < p->n_layers; l++) {
        #if MATRIX_KERNEL_DEBUG_PRINT
        print_uart("[bare] rmsnorm1...\r\n");
        #endif
        TIME_OP(pos, "rmsnorm1", PRINT_DIMS_1(dim), rmsnorm(s->xb, x, w->rms_att_weight + l*dim, dim));
        ...
        #endif
        TIME_OP(pos, "quantize1", PRINT_DIMS_1(dim), quantize(&s->xq, s->xb, dim));
        #if MATRIX_KERNEL_DEBUG_PRINT
        print_uart("[bare] matmul_q...\r\n");
        #endif
        TIME_OP(pos, "wq", PRINT_DIMS_2(dim, dim), MATMUL_Q_TAGGED("wq", (int)l, s->q, &s->xq, w->wq_q + l, dim, dim));
        TIME_OP(pos, "wk", PRINT_DIMS_2(kv_dim, dim), MATMUL_Q_TAGGED("wk", (int)l, s->k, &s->xq, w->wk_q + l, dim, kv_dim));
        TIME_OP(pos, "wv", PRINT_DIMS_2(kv_dim, dim), MATMUL_Q_TAGGED("wv", (int)l, s->v, &s->xq, w->wv_q + l, dim, kv_dim));

        TIME_OP(pos, "rope", PRINT_DIMS_1(dim),
        ...
        TIME_OP(pos, "memcpy_k", PRINT_DIMS_1(kv_dim), memcpy(k_slot, s->k, kv_dim * sizeof(float)));
        TIME_OP(pos, "memcpy_v", PRINT_DIMS_1(kv_dim), memcpy(v_slot, s->v, kv_dim * sizeof(float)));

        for (int h = 0; h < p->n_heads; h++) {
            float* q = s->q + h * head_size;
            float* att = s->att + h * p->seq_len;
            float scale = 1.0f / sqrtf((float)head_size);
            TIME_OP(pos, "att_score", PRINT_DIMS_1(pos + 1),
            for (int t = 0; t <= pos; t++) {
            ...
            TIME_OP(pos, "att_mask", PRINT_DIMS_1(p->seq_len - (pos + 1)),
            for (int t = pos + 1; t < p->seq_len; t++) att[t] = -1e9f;);
            TIME_OP(pos, "att_softmax", PRINT_DIMS_1(pos + 1), softmax(att, pos + 1));
            float* xb = s->xb + h * head_size;
            for (int i = 0; i < head_size; i++) xb[i] = 0.0f;
            TIME_OP(pos, "att_val", PRINT_DIMS_1(pos + 1),
            for (int t = 0; t <= pos; t++) {
                float att_t = att[t];
        ...
        TIME_OP(pos, "quantize2", PRINT_DIMS_1(dim), quantize(&s->xq, s->xb, dim));
        TIME_OP(pos, "wo", PRINT_DIMS_2(dim, dim), MATMUL_Q_TAGGED("wo", (int)l, s->xb2, &s->xq, w->wo_q + l, dim, dim));
        TIME_OP(pos, "resid1", PRINT_DIMS_1(dim), for (int i = 0; i < dim; i++) x[i] = x[i] + s->xb2[i];);

        TIME_OP(pos, "rmsnorm2", PRINT_DIMS_1(dim), rmsnorm(s->xb, x, w->rms_ffn_weight + l*dim, dim));
        TIME_OP(pos, "quantize3", PRINT_DIMS_1(dim), quantize(&s->xq, s->xb, dim));
        TIME_OP(pos, "w1", PRINT_DIMS_2(dim, hidden_dim), MATMUL_Q_TAGGED("ffn", (int)l, s->hb, &s->xq, w->w1_q + l, dim, hidden_dim));
        TIME_OP(pos, "w3", PRINT_DIMS_2(dim, hidden_dim), MATMUL_Q_TAGGED("ffn", (int)l, s->hb2, &s->xq, w->w1_q + l, dim, hidden_dim));
        TIME_OP(pos, "silu", PRINT_DIMS_1(hidden_dim),
        for (int i = 0; i < hidden_dim; i++) {
            float val = s->hb[i];
            val *= 1.0f / (1.0f + expf(-val));
            s->hb[i] = val * s->hb2[i];
        });
        TIME_OP(pos, "quantize4", PRINT_DIMS_1(hidden_dim), quantize(&s->hq, s->hb, hidden_dim));
        TIME_OP(pos, "w2", PRINT_DIMS_2(hidden_dim, dim), MATMUL_Q_TAGGED("ffn", (int)l, s->xb, &s->hq, w->w2_q + l, hidden_dim, dim));
        TIME_OP(pos, "resid2", PRINT_DIMS_1(dim), for (int i = 0; i < dim; i++) x[i] = x[i] + s->xb[i];);
    }

    TIME_OP(pos, "rmsnorm_final", PRINT_DIMS_1(dim), rmsnorm(x, x, w->rms_final_weight, dim));
    TIME_OP(pos, "quantize_final", PRINT_DIMS_1(dim), quantize(&s->xq, x, dim));
    TIME_OP(pos, "wcls", PRINT_DIMS_2(p->dim, p->vocab_size), MATMUL_Q_TAGGED("wcls", -1, s->logits, &s->xq, &w->wcls_q, p->dim, p->vocab_size));
    return s->logits;
}
...
static void matmul_q_batch(float* xout, const int8_t* xq, const float* xs, 
    ...
    matrix_kernel_qmatmul_f32_noblk_batch(xout, xq, xs, wq, ws, n, d, GS, n, batch);
}
...
void transformer_prefill(Transformer *t, int* tokens, int n_tokens) {
    ...
    for (int l = 0; l < p->n_layers; l++) {
        // RMSNorm (Att)
        TIME_OP(0, "prefill_rmsnorm1", PRINT_DIMS_1(dim),
        for (int i = 0; i < n_tokens; i++) {
            rmsnorm(xb_batch + i * dim, x_batch + i * dim, w->rms_att_weight + l*dim, dim);
        });

        // QKV Matmuls (Batched Matrix-Matrix)
        TIME_OP(0, "prefill_quant1", PRINT_DIMS_2(dim, n_tokens), quantize_batch(xq_batch_q, xq_batch_s, xb_batch, dim, n_tokens));
        TIME_OP(0, "prefill_wq", PRINT_DIMS_3(dim, dim, n_tokens), matmul_q_batch(q_batch, xq_batch_q, xq_batch_s, w->wq_q[l].q, w->wq_q[l].s, dim, dim, n_tokens));
        TIME_OP(0, "prefill_wk", PRINT_DIMS_3(kv_dim, dim, n_tokens), matmul_q_batch(k_batch, xq_batch_q, xq_batch_s, w->wk_q[l].q, w->wk_q[l].s, dim, kv_dim, n_tokens));
        TIME_OP(0, "prefill_wv", PRINT_DIMS_3(kv_dim, dim, n_tokens), matmul_q_batch(v_batch, xq_batch_q, xq_batch_s, w->wv_q[l].q, w->wv_q[l].s, dim, kv_dim, n_tokens));

        // RoPE and KV Cache Update (Position-dependent)
        TIME_OP(0, "prefill_rope_cache", PRINT_DIMS_2(dim, n_tokens),
        for (int i = 0; i < n_tokens; i++) {
            int pos = i;
        ...
        // Causal Attention
        TIME_OP(0, "prefill_attention", PRINT_DIMS_2(n_tokens, n_tokens),
        for (int i = 0; i < n_tokens; i++) {
            int pos = i;
            float* layer_k_cache = s->key_cache + (l * p->seq_len * kv_dim);
            float* layer_v_cache = s->value_cache + (l * p->seq_len * kv_dim);
        ...
        TIME_OP(0, "prefill_quant2", PRINT_DIMS_2(dim, n_tokens), quantize_batch(xq_batch_q, xq_batch_s, xb_batch, dim, n_tokens));
        TIME_OP(0, "prefill_wo", PRINT_DIMS_3(dim, dim, n_tokens), matmul_q_batch(xb2_batch, xq_batch_q, xq_batch_s, w->wo_q[l].q, w->wo_q[l].s, dim, dim, n_tokens));
        for(int i=0; i<n_tokens*dim; i++) x_batch[i] += xb2_batch[i];

        // FFN (Batched)
        TIME_OP(0, "prefill_rmsnorm2", PRINT_DIMS_2(dim, n_tokens),
        for (int i = 0; i < n_tokens; i++) {
            rmsnorm(xb_batch + i * dim, x_batch + i * dim, w->rms_ffn_weight + l*dim, dim);
        });
        TIME_OP(0, "prefill_quant3", PRINT_DIMS_2(dim, n_tokens), quantize_batch(xq_batch_q, xq_batch_s, xb_batch, dim, n_tokens));
        TIME_OP(0, "prefill_w1", PRINT_DIMS_3(dim, hidden_dim, n_tokens), matmul_q_batch(hb_batch, xq_batch_q, xq_batch_s, w->w1_q[l].q, w->w1_q[l].s, dim, hidden_dim, n_tokens));
        TIME_OP(0, "prefill_w3", PRINT_DIMS_3(dim, hidden_dim, n_tokens), matmul_q_batch(hb2_batch, xq_batch_q, xq_batch_s, w->w3_q[l].q, w->w3_q[l].s, dim, hidden_dim, n_tokens));
        ...
        TIME_OP(0, "prefill_quant4", PRINT_DIMS_2(hidden_dim, n_tokens), quantize_batch(hq_batch_q, hq_batch_s, hb_batch, hidden_dim, n_tokens));
        TIME_OP(0, "prefill_w2", PRINT_DIMS_3(hidden_dim, dim, n_tokens), matmul_q_batch(xb_batch, hq_batch_q, hq_batch_s, w->w2_q[l].q, w->w2_q[l].s, hidden_dim, dim, n_tokens));
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
    ...
    if (num_prompt_tokens > 1) {
        TIME_OP(-1, "PREFILL_TOTAL", PRINT_DIMS_1(num_prompt_tokens - 1), 
