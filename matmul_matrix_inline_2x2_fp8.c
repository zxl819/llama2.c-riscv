...
#ifndef MATMUL_MATRIX_KERNEL_ONLY
// 打印 fp32 矩阵（按 bit 模式输出），rows x cols，每行结尾换行
static void print_mat_f32_bits(const float* m, int rows, int cols) {
    for (int r = 0; r < rows; ++r) {
        for (int c = 0; c < cols; ++c) {
      uint32_t bits = f32_bits(m[r * cols + c]);
      print_uart("0x");
      print_uart_hex((uint64_t)bits);
...
int main()
{
    ...
    // 打印计算结果 C（fp32）
    print_uart("C result (fp32 bits):\r\n");
    // print_uart("C address:\r\n ");
    // print_uart_hex((uint64_t)(uintptr_t)C);
    // print_uart("\r\n");
    print_mat_f32_bits((const float*)C, 4, 4);

    print_uart("Golden D_ref (fp32 bits):\r\n");
    // print_uart("D_ref address:\r\n ");
    // print_uart_hex((uint64_t)(uintptr_t)D_ref);
    // print_uart("\r\n");
    print_mat_f32_bits((const float*)D_ref, 4, 4);

    int mismatch = 0;
    for (int idx = 0; idx < 4 * 4; ++idx) {
      if (f32_bits(C[idx]) != f32_bits(D_ref[idx])) {
        mismatch++;
      }
    }
    if (mismatch == 0) {
