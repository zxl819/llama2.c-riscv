// Bare-metal friendly inference harness for llama2.c (Chat Mode)
// ---------------------------------------------------
...
// binaries are embedded directly into the ELF via objcopy.
...
int getchar(void); // Added in bare_syscalls.c
void exit(int code);
...
void read_stdin(char* buffer, int max_len) {
    int i = 0;
    while (i < max_len - 1) {
        int c = getchar();
...
void chat(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, int steps) {
    char user_input[256];
    int* prompt_tokens = malloc((steps + 256) * sizeof(int)); // Buffer for tokens
    int num_prompt_tokens = 0;
    int token, next;
    int pos = 0;

    // Initial system prompt or greeting?
    // For now, just start empty or with a greeting
    printf("Llama2 Chat (Bare Metal Spike)\n");
    printf("Type your message and press Enter.\n");

    while (1) {
        // 1. Read User Input
        printf("User: ");
        read_stdin(user_input, sizeof(user_input));
        printf("\n");

        if (strlen(user_input) == 0) continue;

        // 2. Encode User Input
        // We need to append to the current context if we want history, 
        // but for simplicity in this bare metal version, let's just process the new prompt
        // Note: Real chat needs [INST] tags etc for Llama 2 Chat models, 
        // but here we just feed raw text.
        
        // Reset for single-turn (simplest for now) or keep pos for multi-turn?
        // Let's try multi-turn by keeping 'pos' and appending tokens.
        ...
        // 3. Generate Response
        printf("Llama: ");
        // The last token processed was the last token of user input.
        // We need to sample the *next* token based on that.
        // Actually, forward() returns logits for the *next* token prediction.
        // So we need to sample from the logits of the last user token.
        
        // Re-run the last user token to get logits for generation? 
        // Or just use the logits from the loop above.
        // The loop above calculated logits for user_tokens[i].
        // So after the loop, 'logits' (if we captured it) would be for the next token.
        
        // Let's re-run forward for the very last token to get the logits pointer
        ...
        while (pos < steps) {
            ...
            // Print
            if (next == 1) break; // EOS? (Llama 2 usually 2 is EOS, 1 is BOS. Check tokenizer)
            // In run.c: 1=BOS, 2=EOS.
            if (next == 2) break; 
            ...
            token = next;
            logits = forward(transformer, token, pos - 1); // pos was incremented
            gen_len++;
            
            // Stop generation on newline for chat? Or let it generate until EOS?
            // Usually chat models generate until EOS.
            if (next == 2) { printf("\n"); break; }
        }
        printf("\n");
        
        if (pos >= steps) {
            printf("(Context limit reached, resetting)\n");
            pos = 0;
        }
    }
...
int main(void) {
    ...
    chat(&transformer, &tokenizer, &sampler, BARE_STEPS);
