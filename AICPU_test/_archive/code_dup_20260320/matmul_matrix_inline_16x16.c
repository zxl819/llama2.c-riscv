/*
---
title: 16x16 矩阵乘加运算测试 (AME Inline)
date: 2026-01-29
description: 使用 RISC-V 矩阵扩展 (AME) 指令实现的 16x16 int8 矩阵乘加测试例。执行公式为 D = A * B^T + C。
author: zhaoxinlei
version: 1.0
---
============================================================================================================================================================
代码说明
============================================================================================================================================================
该测试例演示了如何使用自定义矩阵扩展指令（mqma.b.mm 等）完成高吞吐量的整数矩阵运算。
主要步骤：
1. 静态分配 64 字节对齐的 A (int8), B (int8), C (int32) 矩阵。
2. 配置矩阵形状：通过 msettilem, msettilen, msettilek 设置 16x16x16 的计算维度。
3. 加载累加器：使用 mlce32.m 加载初始矩阵 C。
4. 加载输入并计算：使用 mlae8.m 和 mlbe8.m 加载分块，执行 mqma.b.mm 进行乘累加。
5. 写回：使用 msce32.m 将结果存回 D。
6. 验证：比对硬件计算结果与预定义的 Golden 数据 D_ref_init。
============================================================================================================================================================
*/
