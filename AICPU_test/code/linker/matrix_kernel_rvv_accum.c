...
// The function implementation - contiguous ws layout
void matrix_kernel_rvv_accum_i32_to_f32_ws_xs(float *dst,
    ...
    const int ws_stride_in_floats = 1;
...
s_accum_call_count++;
    ...
    while (avl > 0) {
        ...
        if (ws_stride_in_floats == 1) {
               vws = __riscv_vle32_v_f32m1((const float *)(ws + offset), vl);
#if MATRIX_KERNEL_DEBUG_PRINT && MATRIX_KERNEL_DEBUG_VERBOSE
             if (s_accum_call_count <= 2) {
                 print_uart("    [rvv_accum] ws_chunk[0..3] (direct): ");
                 for(int k=0; k<4; k++) {
                      print_float_fixed3(ws[offset + k]); print_uart(" ");
                 }
                 print_uart("\r\n");
             }
#endif
             ...
             const volatile uint32_t *curr_ws_ptr_u = (const volatile uint32_t *)(const void *)ws;
