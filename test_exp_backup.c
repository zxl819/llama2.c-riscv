...
static float src_pad[64] = {
    ...
    0.0f // padding element
};

static float golden_pad[64] = {
    ...
    1.000000000e+00f, // padding ref = exp(0)
};
...
int main() {
    ...
    const size_t pad_n = 64;
    float dst[64];
    vector_exp_rvv(dst, src_pad, pad_n);
    debug_delay_cycles(1000000);

    g_log_cnt = 0;
    for (int i = 0; i < N && i < LOG_MAX; ++i) {
        g_log_input[i]  = src_pad[i];
        g_log_output[i] = dst[i];
        g_log_ref[i]    = golden_pad[i];
    }
